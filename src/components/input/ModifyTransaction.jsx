...
const ModifyTransaction = ({
    ...
    const loadTransaction = async (transId) => {
        try {
            const fetchedTransaction = await transactionsService.getById(transId);
            ...
            // First set the transaction type and load its categories
            setTransactionType(fetchedTransaction.type);
            await loadCategories(fetchedTransaction.type);
            
            // Then set the rest of the transaction data
            setTransaction(fetchedTransaction);
            setAmount(fetchedTransaction.amount.toString());
            setNote(fetchedTransaction.note || '');
            setSelectedCategory(fetchedTransaction.categories);
            setDate(new Date(fetchedTransaction.date));
        } catch (error) {
    ...
    const loadCategories = async (typeOverride) => {
        try {
            const typeToUse = typeOverride || transactionType;
            console.log('Loading categories for transaction type:', typeToUse);

            const fetchedCategories = await categoriesService.getAll(typeToUse);
            console.log('Fetched categories:', fetchedCategories);

            if (!fetchedCategories || !Array.isArray(fetchedCategories) || fetchedCategories.length === 0) {
                console.warn('Invalid or empty categories array for transaction type:', typeToUse);
                setCategories([]);
                return;
            }
    ...
    const handleTransactionTypeChange = (type) => {
        ...
        loadCategories(type); // Reload categories for the new type
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await transactionsService.update(transactionId, transactionData);
