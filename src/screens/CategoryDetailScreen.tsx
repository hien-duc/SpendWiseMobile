...
const CategoryDetailScreen = () => {
    ...
    const { categoryId, categoryName, categoryColor, categoryIcon, year } = route.params;
    ...
    const initializeAnimations = (dataLength: number) => {
        const newBarHeights = Array(dataLength).fill(0).map(() => new Animated.Value(0));
    ...
    const fetchCategoryTrend = async () => {
        try {
            ...
            if (error.response?.status === 401) {
                ...
                Alert.alert('Error', 'Unable to fetch category trend data. Please try again.');
            }
        } finally {
            setLoading(false);
        }
