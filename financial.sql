...
create or replace view public.annual_transactions_report as
...
SELECT 
    ...
    sum(coalesce(md.income, 0.00)) over (partition by md.user_id, md.year order by m.month_num) as running_income_total,
    sum(coalesce(md.expense, 0.00)) over (partition by md.user_id, md.year order by m.month_num) as running_expense_total,
    sum(coalesce(md.investment, 0.00)) over (partition by md.user_id, md.year order by m.month_num) as running_investment_total,
    sum(coalesce(md.income, 0.00)) over (partition by md.user_id, md.year) as year_to_date_income_total,
    sum(coalesce(md.expense, 0.00)) over (partition by md.user_id, md.year) as year_to_date_expense_total,
    sum(coalesce(md.investment, 0.00)) over (partition by md.user_id, md.year) as year_to_date_investment_total
FROM months m
LEFT JOIN monthly_data md ON md.month_num = m.month_num
WHERE m.month_num <= extract(month from current_date)
    OR md.year < extract(year from current_date);

-- Create policy for the view
CREATE POLICY annual_transactions_report_policy ON public.annual_transactions_report
    FOR SELECT
    TO authenticated
    USING (user_id = auth.uid());

-- Grant permissions
GRANT SELECT ON public.annual_transactions_report TO authenticated;
GRANT SELECT ON public.transactions TO authenticated;

-- Enable RLS on the view
ALTER VIEW public.annual_transactions_report ENABLE ROW LEVEL SECURITY;
...
-- Add RPC permissions for the report functions
GRANT EXECUTE ON FUNCTION public.get_annual_transactions_report(uuid, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_annual_categories_summary(uuid, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_all_time_balance_report(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_all_time_categories_summary(uuid) TO authenticated;

-- Make sure authenticated users can access the tables used by these functions
GRANT SELECT ON public.transactions TO authenticated;
